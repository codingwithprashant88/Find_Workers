...
function Plumber() {
  ...
  const [ragisterplumber, setRagisterPlumber] = useState({
    name:'',
    age:'',
    phone:'',
    email:'',
  ...
  const allTypes = [
    "Basin & sink (सिंक और बेसिन)",
    "Blockage (ब्‍लॉकेज)",
    "Toilet (टॉयलेट)",
    "Bathroom Fitting (बाथरूम फिटिंग)",
    "Pipe fittings (पाइप फिटिंग)",
    "Testing of Water Lines & Tapping (वाटर लाइन)",
    "Tap & mixer (टैप और मिक्सर)",
    "water RO Connection (R O कनेक्शन)",
    "plumber helper (हेल्पर)",
    "Bath fixing (बाथ फिक्स)",
    "Kitchen Sink (किचन सिंक)",
    "Water tank (पानी टंकी)",
    "Submersible/motor Installation (सब्मर्सिबल/मोटर स्थापना)",
    "Soap Dispenser, Showers, Sensor Taps (टैप, शॉवर्)",
    "Bath Tub Installation (बाथ टब )",
    "Water Tank cleaning (पानी टंकी सफाई)",
  ];

  const handleChange = (e) => {
      const {name, value} = e.target;      
      setRagisterPlumber((prev) => ({
        ...prev, [name]: value
      }))
  }
  ...
  const handleSubmit = async (e) => {
       e.preventDefault();
        ...
        try {
            const {name, age, phone, email, category} = ragisterplumber;
          ...
          "auth-token": token,
              ...
              body: JSON.stringify(ragisterplumber)
            })
        ...
        <div className="relative flex flex-col items-center mt-10">
          <div className="w-screen  bg-blue-100 p-10">
            ...
            <div className="bg-[#f5f4ff] mx-auto flex max-w-6xl flex-col items-center gap-6 rounded-lg bg-neutral-primary-soft p-20 lg:pb-10 lg:pt-10 shadow-sm md:flex-row md:gap-10">
              ...
              <img
                src="https://t4.ftcdn.net/jpg/02/20/20/41/360_F_220204174_vfgB0Vo2i4MZ8Sv5hmtsx5IwcvrDCZox.jpg"
  ...
  <div className="max-w-6xl mx-auto text-center">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6  mb-5">
      ...
      <div key={index} className="bg-yellow-400/60 rounded-2xl shadow-md p-6 text-gray-800 font-semibold text-lg hover:scale-105 transition">
        {item}
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <select   onChange={handleChange}
                  name="category"
                  value={ragisterplumber.category} className="w-full rounded-xl border border-gray-300 px-4 py-2 bg-white focus:ring-2 focus:ring-blue-500 outline-none">

...
function Carpenter() {
  const [ragisterCerpenter, setRagisterCerpenter] = useState({
    name: "",
    age: "",
    phone: "",
    email: "",
  ...
  const allTypes = [
    "    Balcony (बालकनी)",
    "Cupboard & drawer (अलमारी और दराज)",
    "Drill hanging (ड्रिल )",
    "Sofa (सोफ़ा)",
    "Door & Window frames (दरवाज़े और खिड़की का ढांचा)",
    "Television frames (टीवी का ढाँचा)",
    "helper (हेल्पर)",
    "Bed (बेड)",
    "Door (दरवाजे)",
    "Other Furniture (फर्नीचर)",
    "Window (खिड़की)",
    "Interior Works (इन्टीरीअर का काम)",
    "Almira (अलमिरा)",
    "Modular Kitchen (मॉडुलर किचन)",
  ];

  const handleChange = (e) => {
    const { name, value } = e.target;

    setRagisterCerpenter((prev) => ({
      ...prev,
      [name]: value,
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { name, age, phone, email, category } = ragisterCerpenter;
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify(ragisterCerpenter),
      });
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  ...
                  <select
                    onChange={handleChange}
                    name="category"
                    value={ragisterCerpenter.category}

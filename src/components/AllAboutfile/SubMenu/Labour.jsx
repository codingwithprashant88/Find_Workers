...
function Labour() {
  ...
  const labourWork = [
    "Concrete mixer labour (कंक्रीट लेबर)",
    "Construction material shifting helper (सामग्री स्थानांतरण लेबर)",
    "Factory worker helper (फैक्ट्री हेल्पर)",
    "Heavy building worker helper (बड़ी बिल्डिंग हेल्पर)",
    "Lanter worker helper (लेन्टर हेल्पर )",
    "Road construction worker helper (सड़क निर्माण हेल्पर)",
    "Sewage worker helper (सीवेज हेल्पर)",
    "Mason Helper (मिस्त्री हेल्पर)",
    "Loading & Unloading labour (लोडिंग और अनलोडिंग लेबर) ",
    "Stone Crusher (पत्थर तोड़ने वाला)",
    "Mines Excavator (खानों की खुदाई करने वाला)",
    "Gravels Segmentation Labour (बजरी विभाजन लेबर)",
    "Manual Scavangers (मैनुअल स्कवनजर )",
    "Bar bender Helper (सरिया हेल्पर)",
    "shuttering helper (शटरिंग हेल्पर)",
    "others (अन्य)",
  ];
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { name, age, phone, email, category } = registerInfo;
    ...
    try {
      ...
      const { success, message, error } = storeData;
      if (success) {
        handleSuccess(message);
        setTimeout(() => {
          navigate("/login");  // will change into apply card
        }, 1000);
      } else if (error) {
